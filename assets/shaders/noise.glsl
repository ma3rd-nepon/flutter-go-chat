...
void C(int ch) {
    vec2 p = any(greaterThan(abs(f * 8.5 - x - 0.5), vec2(0.5))) ? vec2(0) : f * 8.5 - x;
    float g = 2.0 * (texture(u_tex1, p / 16.0 + fract(vec2(float(ch), 15.0 - floor(float(ch) / 16.0)) / 16.0)).w - 127.0 / 255.0) / w;
    c = mix(c, vec3(0.9), smoothstep(30.0, 15.0, g));
    c = mix(c, vec3(0.0), smoothstep(15.0, 0.0, g));
    x.x += 0.6;
}
...
void main() {
    ...
    else if (id == 15) c.rgb += fbm_scratches12(p, 6);
