...
class _NovaButtonState extends State<NovaButton>
  ...
  Color _accent() {
    switch (widget.variant) {
      case ButtonVariant.primary:
        return NovaTheme.flame;
      case ButtonVariant.secondary:
        return NovaTheme.gold;
      case ButtonVariant.danger:
        return NovaTheme.ember;
      case ButtonVariant.ghost:
        return NovaTheme.paper;
    }
  }

  List<Color> _gradientColors(double breathe) {
    final k = breathe * 0.5;
    switch (widget.variant) {
      case ButtonVariant.primary:
        return [
          Color.lerp(const Color(0xFFFF8A3D), const Color(0xFFFFA45C), k)!,
          Color.lerp(NovaTheme.flame, const Color(0xFFFF7A1F), k)!,
        ];
      case ButtonVariant.secondary:
        return [
          Color.lerp(const Color(0xFFFFD34D), const Color(0xFFFFE080), k)!,
          Color.lerp(NovaTheme.gold, const Color(0xFFFFC22E), k)!,
        ];
      case ButtonVariant.danger:
        return [
          Color.lerp(const Color(0xFFFF6B5E), const Color(0xFFFF8579), k)!,
          Color.lerp(NovaTheme.ember, const Color(0xFFFF5147), k)!,
        ];
      case ButtonVariant.ghost:
  ...
  Color _textColor() {
    if (_disabled) return NovaTheme.muted;
    if (_isGhost) return NovaTheme.paper;
    if (_isOutlined) return _accent();
    switch (widget.variant) {
      case ButtonVariant.primary:
      case ButtonVariant.secondary:
        return NovaTheme.ink;
      case ButtonVariant.danger:
      case ButtonVariant.ghost:
        return NovaTheme.paper;
    }
              ...
              decoration: BoxDecoration(
                gradient: filled
                    ? LinearGradient(
                        ...
                        colors: _gradientColors(breathe),
                      )
                    : null,
                color: _disabled
                    ? NovaTheme.surface
                ...
                border: filled
                    ? Border.all(
                        color: NovaTheme.ink.withValues(alpha: 0.35),
                        width: 1.5,
                      )
                    : _isGhost
