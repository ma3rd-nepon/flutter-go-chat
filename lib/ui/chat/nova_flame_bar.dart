  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        return SizedBox(
          width: widget.width,
          child: CustomPaint(painter: _FlamePainter(_controller.value, colors)),
        );
      },
    );
  }
...
class _FlamePainter extends CustomPainter {
  _FlamePainter(this.t, this.colors);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i <= steps; i++) {
      ...
      final color = Color.lerp(colors.error, colors.primaryHover, k)!;
      canvas.drawRect(
    ...
    for (var s = 0; s < 3; s++) {
      ...
      canvas.drawCircle(
        ...
        Paint()
          ..blendMode = BlendMode.plus
          ..color = colors.primaryHover.withValues(alpha: (1 - p) * 0.9),
      );
    }
  }
